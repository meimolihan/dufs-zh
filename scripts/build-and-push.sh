...
#   release.yml -> 多架构二进制（dufs-<ver>-<target>.tar.gz）创建 GitHub Release
...
info "写入发版备注 RELEASE_NOTES.md"
{
  ...
  printf 'docker pull mobufan/dufs-zh:latest\n'
  printf 'docker pull mobufan/dufs-zh:%s\n' "${TAG}"
  printf '\n'
  printf 'docker pull ghcr.io/meimolihan/dufs-zh:latest\n'
  printf 'docker pull ghcr.io/meimolihan/dufs-zh:%s\n' "${TAG}"
} > RELEASE_NOTES.md
...
echo -e "${gl_bai}远程安装命令： ${gl_lv}bash <(curl -sL gitee.com/meimolihan/cmdbox/raw/master/sh/dc_inst_dufs.sh) 5000 /vol1/1000/compose/dufs-zh${gl_bai}"
